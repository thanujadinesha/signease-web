    ...
    <div>
      ...
      >
        ...
        >
          ...
          <div onMouseDown={onResizeStart} className="absolute bottom-0 right-0 w-4 h-4 bg-accent rounded-tl cursor-se-resize" />
        </div>
      </div>
...
function DownloadStep({
  ...
  const generate = useCallback(async () => {
    try {
      ...
      ctx.drawImage(sigImg, placement.x * scaleX, placement.y * scaleY, placement.w * scaleX, placement.h * scaleY);
...
function QuickSignContent() {
  ...
  function reset() { setSig(null); setDoc(null); setPlacement(null); setStep('signature'); }
    ...
    <div className="min-h-screen">
      <div className="border-b border-border bg-surface/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-6 py-4 flex items-center gap-3">
          ...
          <span className="text-base font-bold text-text1 flex-1">Quick Sign</span>
          {user && (
            <Link href="/profile">
      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
        ...
        {step === 'signature' && <SignatureStep onNext={s => { setSig(s); setStep('document'); }} />}
        {step === 'document'  && <DocumentStep onNext={d => { setDoc(d); setStep('place'); }} onBack={() => setStep('signature')} />}
        {step === 'place' && sig && doc && (
          <PlaceStep sig={sig} doc={doc} onNext={p => { setPlacement(p); setStep('download'); }} onBack={() => setStep('document')} />
        )}
...
export default function QuickSignPage() {
  return <AuthGuard><QuickSignContent /></AuthGuard>;
}
